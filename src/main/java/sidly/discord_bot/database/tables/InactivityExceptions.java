...
public class InactivityExceptions {

    public static Map<String, Long> getAll() {
        Map<String, Long> results = new HashMap<>();
        String sql = "SELECT uuid, exp_timestamp FROM inactivity_exceptions";
    ...
    public static Long get(String uuid) {
        String sql = "SELECT exp_timestamp FROM inactivity_exceptions WHERE uuid = ?";
        try (PreparedStatement stmt = connection.prepareStatement(sql)) {
    ...
    public static void add(String uuid, long expTimestamp) {
        String sql = "INSERT OR REPLACE INTO inactivity_exceptions (uuid, exp_timestamp) VALUES (?, ?)";
        try (PreparedStatement stmt = connection.prepareStatement(sql)) {
            stmt.setString(1, uuid);
